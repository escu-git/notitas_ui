...
import { Button, Grid, Modal, Paper, Select, Switch, TextField, Typography } from '@mui/material';
...
const NoteCreate = ({ modal, fetchAgain }: componentProps) => {
    ...
    const createNote = async(note: NewNoteModel) => {
        try {
            await notesService.postNote(note)
            .then(x=>{
                if(x.status ==200)
                    fetchAgain();
            })
    ...
    return <Modal className={classes.modalNewNote} open={openCreateNoteModal} onClose={handleCloseModal}>
        <Grid container item xs={12} className={classes.createNoteModal}>
            <Paper className={classes.modalPaper}>
                ...
                <Grid container item xs={12} className={classes.modalFormContainer}>
                        ...
                        <Grid container item xs={12} gap={'0.5em'}>
                            ...
                            <Grid container item xs={12} flexDirection='row'>
                                <Grid container item xs={12}>
                                    <Typography variant={'h6'}>{`Reminder: ${reminder ? 'ON' : 'OFF'}`}</Typography>
                                    <Controller
                                ...
                                {reminder &&
                                    <Grid container item xs={12} flexDirection='row' gap={'0.5em'}>
                                        <Grid>
                                            <Controller
                                                name="reminder.date"
                                                control={control}
                                                render={({ field }) => <TextField label='Date' {...field} />
